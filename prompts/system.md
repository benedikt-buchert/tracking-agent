...
3. **Capture dataLayer events** — Use get_datalayer (with from_index to get only new
   events since your last capture) to read window.dataLayer after each interaction.
   This helps you understand what has been tracked so far.

4. **Cover all event types** — Cross-reference the expected event names from the schema
...
- Always call get_datalayer with from_index set to the length of events you have
  already seen, so you only see new events.
